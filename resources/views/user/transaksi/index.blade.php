...
<div class="container mx-auto p-6 flex flex-col lg:flex-row justify-center gap-6">
                ...
                <tbody>
                    ...
                    <tr class="hover:bg-gray-100 text-sm">
                        ...
                        <td class="border border-gray-300 px-4 py-2 flex justify-center gap-2">
                            <a href="{{ route('transaksi.edit', $transaksi->id) }}" class="bg-yellow-500 text-white px-3 py-1 rounded hover:bg-yellow-600 transition">
                                Edit
                            </a>
    ...
    <div class="lg:w-1/4 w-full p-4 bg-gray-100 rounded-lg shadow-md">
        ...
        <form action="{{ route('transaksi.store') }}" method="POST">
            ...
            <button type="submit" class="bg-blue-500 text-white w-full py-2 rounded mt-2 hover:bg-blue-600 transition">
                Tambah
            </button>
        </form>
        ...
        <form action="{{ route('transaksi.hapusSemua') }}" method="POST" class="mt-4">
            @csrf
            @method('DELETE')
            <button type="submit" class="bg-red-500 text-white w-full py-2 rounded hover:bg-red-600 transition"
                    onclick="return confirm('⚠️ Semua transaksi akan dihapus! Anda yakin?')">
                Hapus Semua
            </button>
        </form>
    </div>
